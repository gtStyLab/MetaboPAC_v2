function [absolute_concMatrix predicted_responseFactors] = MetaboPAC_generic_noisy
...
% Assume loaded concentration matrix is in terms of relative abundances
relative_concMatrix = concMatrix;
...
for i = 1:48
    [KEapproach_results(i,:) knownMet] = solveRF_KEapproach_generic(relative_concMatrix,timeVec,knownKinetics);
end
if knownMet ~= 0
...
save('generic_MetaboPAC_results.mat');
end
...
function penalty = calcPenalty(testRF,modelInfo,relative_Vpool,numMetabs,numFlux,MA_reactions,relative_concMatrix,timeVec,fluxTimeVec,RF_kinetics,knownMet,knownKinetics)
    ...
    newFluxMatrixTemp = calcFluxWithKinetics_generic(absolute_concMatrix,timeVec,knownKinetics);
    ...
    if sum(sum(Vcalc))~=0 && ~isnan(sum(sum(Vcalc)))
        ...
        % metabolite
        oneContMetCorr_penalty = pen_oneContMetCorr_generic(absolute_concMatrix,Vcalc,timeVec,fluxTimeVec);
        
        % Calculate curve fit penalty for reactions controlled by one
        % metabolite
        oneContMetCurveFit_penalty = pen_oneContMetCurveFit_generic(absolute_concMatrix,Vcalc,timeVec,fluxTimeVec);

        % Calculate BST fit penalty
        nT = size(absolute_concMatrix,1)-1;
        BST_penalty = pen_BSTfit_generic(nT,absolute_concMatrix,Vcalc,MA_reactions);
