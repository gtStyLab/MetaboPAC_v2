...
function [penalty,massbalance_penalty,conc_penalty,oneContMetCorr_penalty,oneContMetCurveFit_penalty,BST_penalty,ss_penalty,Vcalc_pool,Vcalc]= calcPenalty_ecoli(testRF,modelInfo,relative_Vpool,numMetabs,numFlux,~,relative_concMatrix,timeVec,fluxTimeVec,RF_kinetics,knownMet,knownKinetics)
    ...
    fluxMatrixTemp = calcFluxWithKinetics_chass(absolute_concMatrix,timeVec,knownKinetics);
    ...
    if sum(sum(Vcalc))~=0 && ~isnan(sum(sum(Vcalc)))
        ...
        BST_penalty = pen_BSTfit_chass_updated(absolute_concMatrix,Vcalc);

        %Calculate steady state penalty
        ss_penalty = pen_devSSdist_ecoli(Vcalc(:,1:numFlux));
        % Calculate total penalty
        penalty = abs(massbalance_penalty) + abs(conc_penalty) + abs(oneContMetCorr_penalty) + abs(oneContMetCurveFit_penalty) + abs(BST_penalty) + abs(ss_penalty);
        if isnan(penalty)
            penalty = 1e7;
        end
    else
